...
class Orchestrator:
    ...
    async def _on_bar(self, event: BarEvent) -> None:
        ...
        for name, strategy in self._strategies.items():
            ...
            if result.signal:
                ...
                result.signal.correlation_id = event.correlation_id or event.event_id
    ...
    async def _process_signal(self, signal: SignalEvent) -> None:
        ...
        if not passed:
            for block in blocks:
                ...
                logger.warning("signal_blocked", rule=block.rule_that_blocked, reason=block.reason)
            return
    ...
    async def _on_fill(self, fill: FillEvent) -> None:
        ...
        logger.info("fill_received", symbol=fill.symbol, price=fill.fill_price, qty=fill.quantity)
